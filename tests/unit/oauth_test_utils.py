    ...
    def __call__(self, request, uri, response_headers):
        ...
        if authorization and authorization.replace("Bearer ", "") in self.tokens:
            ...
            return [
                ...
                {
                    "Www-Authenticate": (
                        f'Bearer realm="Trino", token_type="JWT", Bearer x_token_server="{self.token_server}"'
                    ),
                    "Basic realm": '"Trino"',
                },
                "",
...
class MultithreadedTokenServer:
    ...
    def post_statement_callback(self, request, uri, response_headers):
        ...
        return [
            401,
            {
                "Www-Authenticate": f'Bearer x_redirect_server="{redirect_server}", x_token_server="{token_server}"',
                "Basic realm": '"Trino"',
            },
            "",
        ]
