...
```shell
TRINO_RUNNING_HOST=localhost TRINO_RUNNING_PORT=8080 \
    uv run --frozen pytest tests/integration
```

You can also bring the test stack up by hand (useful for poking at it with
`trino-cli` or `psql`-style clients):
